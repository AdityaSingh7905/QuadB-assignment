...
export default function TaskItem({
  ...
  return (
    <div className="flex items-center justify-between p-3 mb-2 border-b dark:border-white last:border-none bg-gray-100 dark:bg-[#2D3C2E] rounded-lg">
      <div className="flex items-center space-x-3">
        <button onClick={() => toggleCompleteHandler(task._id)}>
          {task.isCompleted ? (
            <FaCheckSquare className="text-green-500 text-xl" />
          ) : (
            <FaRegSquare className="text-gray-500 dark:text-white text-xl" />
          )}
        </button>
